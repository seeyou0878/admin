...
class Store{
	...
	function __construct(){
		...
		$obj = new \Yapa(
			...
			/*col_ch*/
			array('代碼', '分站', '類型', '訂單編號', '建立時間', '會員', '金額', '手續費', '付款方式', '付款帳號', '付款餘額', '付款狀態', '儲值目標', '儲值帳號', '儲值餘額', '儲值狀態', '狀態', '警告', '備註', '最後處理', '處理時間'),
			/*empty check*/
			...
			array(
				...
				'',
				'',
				't_order_store_src,alias,id,{"id[!]": [1,5]}',
				'',
				'',
				't_order_paid,alias,id',
				't_game,name,id',
				'',
				'',
			...
			array(
				...
				'hidden',
				'col-md-3 col-sm-3 hidden-xs hidden-modify',
				'hidden',
				'hidden',
			...
			array(
				...
				'hidden', 'text,{"disabled": true}', 'hidden', 'autocomplete', 'text', 'hidden', 'select', 'hidden', 'hidden', 'select', 'select', 'hidden', 'hidden', 'hidden', 'select', 'text', 'textarea', 'hidden', 'hidden'
			),
			/*authority check*/
			array(
			...
			switch($obj->act){
				case 'review':
					...
					foreach($r['data'] ?? [] as $k=>$v){
						...
						$html3 = $tpl->block('store_src')->assign([
							'src_id' => $v['src_id'],
							'src_text' => $v['src_text'],
							'src_status' => $v['src_status'],
						])->render(false);
						
						$html4 = $tpl->block('store_tar')->assign([
							'tar_id' => $v['tar_id'],
					...
					break;
				case 'create':
					$order = \App::make('Lib\Order')->store($obj->arg['data']);
