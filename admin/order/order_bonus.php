...
class Bonus{
	...
	function __construct(){
		...
		$obj = new \Yapa(
			...
			/*col_ch*/
			array('代碼', '分站', '類型', '訂單編號', '建立時間', '會員', '金額', '手續費', '來源目標', '來源說明', '來源餘額', '來源狀態', '儲值目標', '儲值帳號', '儲值餘額', '儲值狀態', '狀態', '警告', '備註', '最後處理', '處理時間'),
			/*empty check*/
			...
			array(
				...
				'hidden', 'text,{"disabled": true}', 'hidden', 'autocomplete', 'text', 'hidden', 'select', 'text', 'hidden', 'hidden', 'select', 'hidden', 'hidden', 'hidden', 'select', 'text', 'textarea', 'hidden', 'hidden'
			),
			/*authority check*/
			array(
			...
			switch($obj->act){
				case 'review':
					...
					foreach($r['data'] ?? [] as $k=>$v){
						...
						$html3 = $tpl->block('bonus_src')->assign([
							'src_id' => $v['src_id'],
							'src_text' => $v['src_text'],
						])->render(false);

						$html4 = $tpl->block('bonus_tar')->assign([
							'tar_id' => $v['tar_id'],
							'tar_text' => $v['tar_text']? '-' . $v['tar_text']: '',
							'tar_status' => $v['tar_status'],
							'tar_value' => number_format($v['tar_value']),
						])->render(false);
					...
					break;
				case 'create':
					$order = \App::make('Lib\Order')->bonus($obj->arg['data']);
					if($order['code']){
						// fail
					}else{
						// 自動嘗試轉移
						...
						$arr['status_id'] = 2;
						$order = \App::make('Lib\Order')->bonus($arr);
					}
